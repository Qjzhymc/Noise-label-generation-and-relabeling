...
if __name__=='__main__':
	#1.获取数据集，添加噪声标签
	dataset=arff.loadarff('birds-train.arff')#数据593行，78列，
	data = pd.DataFrame(dataset[0],dtype=np.float) #获取emotions.arff文件中的data属性的数据，前面都是说明属性的文字不需要
	data=data.values
	X=data[:,:-21]#提取前72列作为特征
	y=data[:,-1:]#提取最后1列作为添加噪声的标签
	z=data[:,-21:-1]#其他不需要动的特征加入z
	print(z.shape)#322行20列
	...
	#train_data加入s，并写入文件
	train_data['s']=s
	train_data.to_csv('birds.csv',index=None)
